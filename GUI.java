...
class GUI extends JFrame implements ActionListener      // implementing ActionListener to handle action
{
	...
	GUI(String message)     //constructor
	{
		super(message);
		label = new JLabel();
		add(label);
	...
	public void actionPerformed(ActionEvent e)   // 'actionPerformed' is the abstract method of ActionListener
	{
		if (e.getSource() == btnNext)   // 'getSource()' Returns the object on which the event occurred. 
		{		// We can use the getSource method to determine which component sourced the event when the listener is registered
			if (check())
				count = count + 1;
			current++;
			set();
		...
		if (e.getActionCommand().equals("Bookmark"))   //  ' getActionCommand()' Returns the command string associated with this action. 
		{
	...
	{
		GUI obj = new GUI("Multiple Choice Questions");
	}
